#!/usr/bin/env python3
"""Phase 6 sec.2 - COPIA VERBATIM (zero semantic drift) di
server/research_scripts/phase5/build_events.py, applicata al dataset di
TRUE HOLDOUT. Stesso identico CFG dict, stesse funzioni di detection -
solo i path di I/O sono cambiati. Gli eventi rilevati nel buffer di
warmup (is_buffer=true) vengono rilevati normalmente ma filtrati DOPO,
nello script di test H006, prima di qualunque calcolo di outcome.

Event layer originale (Phase 5.C): 9 famiglie di eventi rilevate sullo
stesso dataset H4/Market State, indipendentemente da qualunque strategia
...
DATA_DIR = os.path.join(ROOT, "server", "research_scripts", "phase6", "data")
BARS_PATH = os.path.join(DATA_DIR, "xauusd_h4_bars_holdout.csv")
STATE_PATH = os.path.join(DATA_DIR, "market_state_dataset_holdout.csv")
OUT_PATH = os.path.join(DATA_DIR, "events_holdout.csv")
META_PATH = os.path.join(DATA_DIR, "events_holdout.meta.json")
...
def main():
    ...
    meta = {
        ...
        "source_state": "server/research_scripts/phase6/data/market_state_dataset_holdout.csv",
        "detectors_declared_ex_ante": True,
        "built_at": datetime.now(timezone.utc).isoformat(),
        "script": "server/research_scripts/phase6/build_events_holdout.py (copia verbatim di server/research_scripts/phase5/build_events.py, solo path I/O diversi, stesso CFG dict)",
    }
