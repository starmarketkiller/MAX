...
dataset di TRUE HOLDOUT (server/research_scripts/phase6/data/xauusd_h4_bars_holdout.csv)
invece che al dataset di Phase 5. Solo i path di I/O sono cambiati -
formule, parametri P, garanzie causali IDENTICI, non ricopiati a mano
riga per riga ma duplicati esattamente dal file originale.

Market State Dataset v1: feature causali barra-per-barra su
XAUUSD H4.
...
OUT_PATH = os.path.join(DATA_DIR, "market_state_dataset_holdout.csv")
META_PATH = os.path.join(DATA_DIR, "market_state_dataset_holdout.meta.json")
...
def build():
    ...
    meta = {
        "schema_version": 1,
        "source_bars": "server/research_scripts/phase6/data/xauusd_h4_bars_holdout.csv",
        "n_rows": len(out),
        ...
        "script": "server/research_scripts/phase6/build_market_state_holdout.py (copia verbatim di server/research_scripts/phase5/build_market_state.py, solo path I/O diversi)",
    }
