...
//+------------------------------------------------------------------+
// Phase 7.9H - NXS_BreakoutAccDealExportDiagnostic.mq5                |
// Copia BYTE-IDENTICA di NEXUS_EA_v2.mq5 (stesso #include, nessuna    |
// riga di logica di trading/strategia toccata) con UN'UNICA aggiunta: |
// NXS_DIAG_ExportDeals(), chiamata a fine OnTester(), sola lettura,   |
// che esporta ogni deal reale dalla history del Tester tramite        |
// HistoryDealGetDouble(DEAL_PRICE) - nessun impatto su alcuna         |
// decisione di trading gia' presa durante il run. NEXUS_EA_v2.mq5     |
// stesso resta non modificato (verificabile via diff/hash).           |
//+------------------------------------------------------------------+
...
// Phase 7.9H - sola lettura della history del Tester gia' chiusa. Non invia
// ordini, non legge/scrive stato di strategia, non tocca g_breakoutAccState
// ne' alcuna variabile usata da NXS_Strat_*(). Esporta OGNI deal (di
// qualunque strategia/magic) cosi' il join a valle puo' filtrare per
// commento/strategy senza assumere nulla qui.
void NXS_DIAG_ExportDeals(){
   string fname = "NEXUS\\nxs_diag_deals_export.csv";
   int h = FileOpen(fname, FILE_WRITE|FILE_CSV|FILE_ANSI, ',');
   if(h == INVALID_HANDLE){
      PrintFormat("[NXS_DIAG] impossibile aprire %s err=%d", fname, GetLastError());
      return;
   }
   FileWrite(h, "deal_ticket", "position_id", "order_ticket", "time", "time_msc",
   ...
   for(int i = 0; i < total; i++){
      ...
      FileWrite(h,
         (long)d,
         (long)HistoryDealGetInteger(d, DEAL_POSITION_ID),
         (long)HistoryDealGetInteger(d, DEAL_ORDER),
         TimeToString((datetime)HistoryDealGetInteger(d, DEAL_TIME), TIME_DATE|TIME_SECONDS),
         (long)HistoryDealGetInteger(d, DEAL_TIME_MSC),
         HistoryDealGetString(d, DEAL_SYMBOL),
         (long)HistoryDealGetInteger(d, DEAL_MAGIC),
         EnumToString((ENUM_DEAL_TYPE)HistoryDealGetInteger(d, DEAL_TYPE)),
         ...
         DoubleToString(HistoryDealGetDouble(d, DEAL_SL), _Digits),
         DoubleToString(HistoryDealGetDouble(d, DEAL_TP), _Digits),
   ...
   FileFlush(h);
   FileClose(h);
   PrintFormat("[NXS_DIAG] esportati %d deal reali -> %s", total, fname);
}

double OnTester(){
   NXS_DIAG_ExportDeals();
