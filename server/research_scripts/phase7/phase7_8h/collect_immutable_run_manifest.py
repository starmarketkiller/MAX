#!/usr/bin/env python3
"""Phase 7.8H - Raccoglie e hasha gli output RAW del secondo Serious
validation SUBITO dopo la fine del Tester, PRIMA di qualunque
interpretazione (punto 6 dell'istruzione). Il run 1 (7.8G, 0 trade)
resta archiviato separatamente e immutato - non toccato qui."""
import glob
import os
import shutil
import sys
...
OUT_DIR = os.path.join(PHASE78H_DIR, "immutable_run_output")


def find_report():
    candidates = glob.glob(os.path.join(TERM_DATA, "**", "volbrk_serious_3y_run2*"), recursive=True)
    return candidates
...
def main():
    ...
    if os.path.isfile(trades_csv):
        dest = os.path.join(OUT_DIR, "NEXUS_trades_serious_3y_run2.csv")
    ...
    ini_src = r"C:\Users\User\.claude\jobs\703d44b4\tmp\volbrk_serious_3y_run2.ini"
    if os.path.isfile(ini_src):
        dest = os.path.join(OUT_DIR, "volbrk_serious_3y_run2.ini")
    ...
    reseal_doc = load_json(os.path.join(PHASE78H_DIR, "volatility_breakout_atomic_reseal_before_run2_v1.json"))
    collected["atomic_reseal_before_run2_hash"] = reseal_doc["canonical_sha256"]

    prior_run1_manifest = load_json(os.path.join(PHASE7_DIR, "phase7_8g", "immutable_run_manifest_v1.json"))

    payload = {
        "phase": "7.8H",
        "artifact_role": "IMMUTABLE_RUN_MANIFEST_RUN2",
        "candidate_id": "VOLATILITY_BREAKOUT_CONFIRMED",
        "collected_before_any_interpretation": True,
        "collected_files": collected,
        "prior_run1_archived_reference": {
            "canonical_sha256": prior_run1_manifest["canonical_sha256"],
    ...
    out_path = os.path.join(PHASE78H_DIR, "immutable_run_manifest_run2_v1.json")
