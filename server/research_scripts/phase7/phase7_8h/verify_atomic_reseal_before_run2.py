#!/usr/bin/env python3
"""Phase 7.8H - Verifica INDIPENDENTE dell'atomic reseal prima del secondo
run. Ricalcola dai file reali su disco: i 27 hash tick, lo snapshot H4,
l'identita' ambiente (EA EX5/source, timezone), il config corretto (un
solo campo di differenza), e le catene di provenienza verso 7.8F/7.8G.
Fallisce chiuso su qualunque mismatch. MAI esegue il Serious validation."""
import os
import sys
...
MANIFEST_PATH = os.path.join(PHASE78H_DIR, "volatility_breakout_atomic_reseal_before_run2_v1.json")
...
def verify():
    ...
    checks["corrected_config_hash_matches_real_file"] = (
        p["source_artifacts_untouched"]["corrected_config"]["canonical_sha256"] == corrected_doc["canonical_sha256"]
    )

    # ---- Config corretto: esattamente un campo di differenza, ricalcolato ----
    cc = corrected_doc["payload"]["tester_execution_config_corrected"]
    frozen_78f_text = prior_78f_doc["payload"]["tester_execution_config_final"]["raw_text"]
    recomputed_lines = frozen_78f_text.splitlines()
    idx = recomputed_lines.index("InpUseStrategyProfiles=true") + 1
    recomputed_text = "\n".join(recomputed_lines[:idx] + ["InpStrat_VolBreakoutConfirmed=true"] + recomputed_lines[idx:])
    checks["corrected_config_text_recomputed_matches"] = recomputed_text == cc["raw_text"]
    checks["corrected_config_hash_recomputed_matches"] = canonical_sha256({"tester_config_text": recomputed_text}) == cc["sha256"]
    checks["corrected_config_only_one_line_added"] = (
        len(recomputed_text.splitlines()) == len(frozen_78f_text.splitlines()) + 1
    )
    checks["corrected_config_hash_matches_seal"] = cc["sha256"] == p["final_seal"]["corrected_tester_config_hash"]

    # ---- Window-aware fingerprint: ricalcolato dai file REALI ----
    ...
    recomputed_combined = canonical_sha256({
        "tick_files": tick_entries, "h4_snapshot": {k: v for k, v in snap.items()},
    })
    ...
    # ---- Environment identity: ricalcolata dai file/misure REALI ----
    ...
    checks["ea_ex5_hash_recomputed_matches"] = (
        os.path.isfile(ex5_path) and file_sha256(ex5_path) == ei["ea_ex5_sha256"]
    )
    src_path = os.path.join(ROOT, "MQL5", "Experts", "NEXUS_EA_v2.mq5")
    checks["ea_source_hash_recomputed_matches"] = (
        os.path.isfile(src_path) and file_sha256(src_path) == ei["ea_source_sha256"]
    )
    checks["environment_hash_matches_seal"] = canonical_sha256(ei) == p["final_seal"]["environment_identity_hash"]

    # ---- Correction classification: ricontrollata ----
    cls = corrected_doc["payload"]["correction_classification"]
    checks["correction_is_technical_enablement"] = cls["correction_type"] == "TECHNICAL_ENABLEMENT_FIX"
    checks["correction_does_not_change_strategy"] = (
        cls["changes_strategy_identity"] is False and cls["changes_strategy_parameters"] is False
        and cls["changes_verdict_rules"] is False and cls["changes_data_window"] is False
    )
    checks["prior_run_not_reinterpreted"] = cls["prior_run_classification"]["classification"] == "TECHNICALLY_INVALID_ZERO_TRADE_RUN"
