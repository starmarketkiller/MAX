#!/usr/bin/env python3
"""Phase 7.0B sec.10 - downloader tick BID/ASK reali Dukascopy XAUUSD per
il periodo mai acquisito prima: 2023-02-04 -> ultimo giorno pienamente
disponibile (buffer di sicurezza rispetto ad "oggi"). Vedi
acquisition_freeze_declaration_v1.json, scritto e committato PRIMA di
questo fetch.

Stessa identica logica del downloader v2 di Phase 4/Phase 6
(server/research_scripts/phase_h_dukascopy_downloader.py,
server/research_scripts/phase6/phase6_holdout_downloader.py) - copiata
qui SENZA modifiche funzionali, solo con START/END/ROOT diversi, per non
toccare/mischiare la cache/manifest delle fasi precedenti.
"""
...
# Buffer di 2 giorni rispetto a "oggi" (2026-09-18) per non richiedere
# dati non ancora pubblicati da Dukascopy.
END = datetime(2026, 9, 16, tzinfo=timezone.utc)

ROOT = os.path.join(os.path.dirname(os.path.abspath(__file__)), "data_cache_new_period")
...
def fetch_hour_raw(dt: datetime, timeout: int = 20, retries: int = 5) -> tuple[str, bytes | None]:
    ...
    for attempt in range(retries):
        try:
            r = session.get(url, headers={"User-Agent": "Mozilla/5.0 NEXUS-research-phase7"}, timeout=timeout)
    ...
    print(f"[phase7] FAILED {dt.isoformat()}: {str(last_err)[:120]}", flush=True)
    return "failed", None
...
def main():
    ...
    while day <= end:
        ...
        if i % 10 == 0 or not entry["complete"]:
            ...
            print(f"[phase7] {i}/{n_days} {day.date()} ticks={entry['n_ticks']} "
    ...
    save_manifest(manifest)
    print(f"[phase7] COMPLETATO {n_days} giorni in {time.time()-t0:.0f}s", flush=True)
