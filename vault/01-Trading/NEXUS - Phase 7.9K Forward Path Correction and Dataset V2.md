...
| Eventi riclassificati | — | **5/47 (10.6%)** |
...
**Il pattern direzionale (BUY≫SELL) replica in direzione** dopo la correzione — evidenza che non era un artefatto puro dell'offset. Ma **l'ampiezza si è attenuata per SELL** (raddoppiata dal 9.1% al 20.0%), e la scoperta che il 10.6% delle classificazioni cambia con un fix minore, più il cambio di segno nel gate diagnostic controfattuale, sono **segnali di fragilità metodologica** che non c'erano stati dichiarati prima con questa precisione.
...
**Verdetto rivalutato, non ereditato**: il pattern descrittivo (asimmetria BUY/SELL) **sopravvive** alla correzione (evidenza contro un artefatto puro), ma la fragilità aggiuntiva scoperta (10.6% di riclassificazioni, sign-flip nel gate diagnostic) impedisce di alzare la decisione a `MECHANISM_SUPPORTED`. Nessun elemento emerge per abbassarla a `MECHANISM_NOT_SUPPORTED` o `INSUFFICIENT_EVIDENCE` (il pattern direzionale principale è sopravvissuto nella direzione). **Decisione: `MECHANISM_PARTIALLY_SUPPORTED`** (stessa di Phase 7.9J, ma ri-derivata esplicitamente da zero, non mantenuta per inerzia).
...
3. **Fragilità metodologica dimostrata**: 10.6% di riclassificazioni da un fix minore — qualunque futura analisi a singolo-orizzonte va letta con questa cautela esplicita.
...
- **Suite propria 7.9K (pytest)**: 36/36 PASS
- **Suite pytest Phase 7 totale**: **380/380 PASS, 0 fallimenti** (344 precedenti + 36 nuovi)
...
FORWARD PATH CORRECTION: offset di +1 barra confermato in 7.9J,
        ...
        precedente al fill). Impatto: 5/47 riclassificazioni (10.6%),
        1 evento censurato, BROKER_REJECT cambia segno - NON neutrale
