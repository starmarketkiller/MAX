...
## Adapter (piano compile/build — follow-up, non in questo PR)

Il JSON è la fonte; gli adapter si **generano** da lui:
- **MQL5**: un `NXS_StrategyRegistry.mqh` generato (id + selector_index) per
  sostituire le liste sparse — richiede compilazione desktop.
- **Frontend**: sostituire il `const STRATEGIES = [...]` hardcoded in
  `StrategyChainPage.jsx` con un fetch di `/api/strategies/registry` — richiede build.
- **backtest**: generare i nomi da `live_ids()` + alias invece di `STRAT_NAMES_36`.

## Verifica

`python3 contracts/validate_registry.py` → validazione OK, riconciliazione OK.
`server/tests/test_strategy_registry.py` → 11 test (conteggi, alias, unknown-raises,
research-only, selector unici, endpoint). Suite backend completa: 20/20.
