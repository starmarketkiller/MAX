...
- Suite backend completa: **37/37**. Generatore idempotente.

## Follow-up (compile/build, non in questo PR)

- **Frontend**: sostituire i fallback numerici con generazione/validazione dal
  `GET /api/settings/schema` (elimina i `NaN` dai form).
- **EA/MQL5**: allineare i default di `NXS_Inputs.mqh` al contratto (un check di
  drift generato può confrontarli).
